...
static void dessin(void) {
  ...
  for(a = 0; a < M_PI / 2.0f; a += 0.02f) {
    ...
    GLubyte r = rand() % 10, g = rand() % 10, b = rand() % 10;
...
static void initText(GLuint * ptId, const char * text) {
  ...
  if( !(font = TTF_OpenFont("DejaVuSans-Bold.ttf", 200)) ) {
...
static void draw(void) {
  const GLfloat inclinaison = -80.0; //-85.0
  static GLfloat t0 = -1;
  GLfloat t, d;
  if(t0 < 0.0f)
    t0 = SDL_GetTicks();
